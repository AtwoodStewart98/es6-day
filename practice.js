...
let { arrays, objects } = destructuring;
...
function whatToUse(past, present, future) {
  ...
  console.log(
    `Use ${present} when browser support is not an issue, or you can transpile your code.`
  );
...
function evenOdd(...params) {
  ...
  for (var key in params) {
    if (params[key] % 2) {
      obj.odd.push(params[key]);
    } else {
      obj.even.push(params[key]);
    }
  }
  return obj;
}
...
function multiply(num, ...addNums) {
  var arr = [];
  for (var i = 0; i < addNums.length; i++) {
    arr.push(addNums[i] * num);
  }
  return arr;
}
...
let mooresBits = bits.map(val => val * 2);
...
const that = {
  ...
  apply:
    "invokes a function with the specified context, passing an array of arguments individually"
};
